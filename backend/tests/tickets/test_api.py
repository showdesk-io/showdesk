...
class TestTagAPI:
    ...
    def test_list_tags_filters_by_organization(self, authenticated_client, organization) -> None:
    ...
    def test_create_tag(self, authenticated_client, organization) -> None:
        ...
        response = authenticated_client.post("/api/v1/tags/", {
            "name": "Bug",
            "color": "#EF4444",
        })
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_set_tags_replaces_existing(self, authenticated_client, organization) -> None:
...
class TestAttachmentValidation:
    ...
    def test_reject_oversized_file(self, authenticated_client, organization, monkeypatch) -> None:
        ...
        ticket = TicketFactory(organization=organization)
        small_file = SimpleUploadedFile("large.pdf", b"x" * 20, content_type="application/pdf")
...
class TestPriorityLevelAPI:
    ...
    def test_create_custom_priority(self, authenticated_client, organization) -> None:
        ...
        response = authenticated_client.post("/api/v1/priorities/", {
            "name": "Blocker",
            "slug": "blocker",
            "color": "#DC2626",
            "position": 4,
    ...
    def test_update_priority_color(self, authenticated_client, organization) -> None:
        ...
        priority = PriorityLevel.objects.get(
            organization=organization, slug="high",
        )
        response = authenticated_client.patch(
    ...
    def test_priorities_filtered_by_organization(self, authenticated_client, organization) -> None:
    ...
    def test_ticket_uses_custom_priority(self, authenticated_client, organization) -> None:
        """Tickets can use custom priority slugs."""
        PriorityLevelFactory(
            organization=organization, name="Blocker", slug="blocker", position=4,
        )
